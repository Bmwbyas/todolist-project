...
export const Login = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values,formicHelpers) => {
            const action=await dispatch(loginAuth(values))

            if(loginAuth.rejected.match(action)){
                if(action.payload?.fieldsErrors?.length){
                    const error=action.payload.fieldsErrors[0]
                    formicHelpers.setFieldError(error.field,error.error)
                }
                return
            }
